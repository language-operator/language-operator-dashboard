...
.PHONY: dev dev-image dev-secrets dev-apply dev-forward dev-down dev-logs dev-rebuild
...
# Create/update the dev config secret, pulling DATABASE_URL from the Helm release
dev-secrets:
	$(eval DB_URL := $(shell kubectl get secret -n $(NAMESPACE) language-operator-dashboard \
		-o jsonpath='{.data.DATABASE_URL}' 2>/dev/null | base64 -d))
	@if [ -z "$(DB_URL)" ]; then \
		echo "ERROR: Could not read DATABASE_URL from secret 'language-operator-dashboard' in namespace '$(NAMESPACE)'"; \
		echo "Ensure the language-operator Helm chart is deployed first: helm status language-operator -n $(NAMESPACE)"; \
		exit 1; \
	fi
	kubectl create secret generic dashboard-dev-config \
		...
		--from-literal=LANGOP_ORGANIZATION_NAMESPACE_PREFIX="language-operator-" \
		--namespace=$(NAMESPACE) \
